# 팀 통합 발표 대본 — LLM for OS (W11~W12 진행상황 데모)

분량: 약 6분. 슬라이드 7장 (아키텍처 + 4인 슬라이스 + W12 평가 + 향후 계획).
...
OS의 4가지 핵심 개념 — **스케줄러, 프로세스, 시스템콜, 스레드** — 을 한 명씩 나눠 맡았고, W11까지 각자 슬라이스를 구현해 *Hello World* 시연을 통과했습니다. W12에는 baseline / heuristic / Solar LLM 3-way 정량 비교를 마쳤고, 오늘 그 결과를 함께 보여드립니다.
...
W10에서 합의한 큰 그림입니다. 사용자의 자연어가 Solar API를 거쳐 JSON으로 구조화되고, 호스트 측에서 검증과 안전 가드를 통과한 뒤에 xv6 안의 4개 슬라이스로 흘러갑니다. **LLM은 절대 커널 안에 들어오지 않습니다** — 시스템콜 경계에서 작은 정수나 짧은 명령으로 축약된 뒤에만 커널이 받아들입니다.
...
W12에서는 5개 워크로드 × 3 모드 = 15회 QEMU 실험을 돌렸습니다. 결과가 의외로 의미 있어요. **단일 워크로드(cpu_heavy, io_heavy)는 baseline MLFQ가 이미 최적**입니다 — 우리가 구현한 sleep retention이 이미 잘 작동하기 때문이죠.
...
이건 *"비싼 LLM 호출이 실제로 가치 있는 지점"* 을 정량적으로 보여주는 결과입니다. 차트는 `docs/charts/w12/`에 commit되어 있습니다.
...
| 주차 | 작업 |
|---|---|
| **W13** | 4명 슬라이스 통합 — syscall 번호 재배정 + MLFQ 단일화 + NL 브리지 통합 |
| **W14** | 영문 기술 보고서 + 영문 발표 슬라이드 + 백업 데모 영상 |

### 무엇을 말하는가

W13에서는 네 명의 코드를 하나의 xv6 트리에 통합합니다. 가장 큰 작업은 syscall 번호 재배정과 두 MLFQ 구현 중 본인 것으로 단일화, 그리고 세 개의 NL 브리지를 하나로 합치는 일입니다. W14에 영문 보고서와 데모 영상을 마무리할 계획입니다.

---

## 마무리

지금까지 W11 구현 + W12 정량 평가 결과를 보여드렸습니다. *"LLM이 항상 더 좋다"* 가 아니라 *"LLM이 가치 있는 워크로드를 찾았다"* 는 정직한 결과를 얻었습니다. 질문 받겠습니다. 감사합니다.

---

## 📋 발표 운영 메모

- **시간 배분 (총 ~6분):** 도입 20s · 아키텍처 30s · 본인 60s · haneol 50s · minju 50s · jinhwan 50s · W12 70s · 향후 30s · 마무리 20s
- **W12 결과 차트:** `docs/charts/w12/{cpu_heavy,io_heavy,mixed,three_way,realprog}/avg_turnaround.png` 사용 권장
