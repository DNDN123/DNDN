...
found:
  ...
  // === minju: zero per-syscall trace counters (K1 fix) ===
  // Without this, proc-slot reuse leaks the previous tenant's traced=1 and
  // counts into the new process, producing garbage tracetool output.
  p->traced = 0;
  memset(p->syscall_count,  0, sizeof(p->syscall_count));
  memset(p->syscall_errors, 0, sizeof(p->syscall_errors));

  // === jinhwan: thread fields (K1 fix) ===
  // freeproc already resets these, but belt-and-braces in allocproc too.
  p->is_thread = 0;
  p->tgroup    = 0;
...
kforkpri(int level)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->trace_enabled = p->trace_enabled;
  np->traced = p->traced;          // K3 fix: minju trace inheritance
  np->priority = level;
...
// ============================================================
// kthread_create(fcn, arg, ustack) — create a thread sharing caller's VM.
// Returns thread tid on success, -1 on error.
int
kthread_create(uint64 fcn, uint64 arg, uint64 ustack)
{
  ...
  // Copy trapframe so thread inherits registers, then override entry.
  *(np->trapframe) = *(p->trapframe);
...
// Atomically check *uaddr == expected; if so sleep until futex_wake(uaddr).
int
