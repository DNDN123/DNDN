...
or your fine-tuned model running locally (Mac MPS / Windows+CUDA / CPU).
    ...
    python3 nlos.py                   # auto: on-device model if present, else solar, else offline
...
Backend selection is the ONLY thing that changes between on-device and Solar —
the bridge, guard, kernel and xv6 commands are identical either way. The default
prefers the on-device fine-tuned model; Solar is a fallback when no local model
is available (or use --backend solar to force it).
"""
...
def choose_backend(args) -> str:
    if args.backend != "auto":
        return args.backend
    # Prefer the on-device fine-tuned model over cloud Solar. Solar is only used
    # as a fallback when no local model is available (or via --backend solar).
